...
def create_python_gantt():
    ...
    # merge
    protocol.add_children([onchain,git_tellor,git_chainlink,alt_chains, ci, security])
    ...
    bounties = Activity(description="Subsidize bounties", duration=10, new_tag=2, parent=platform_eco)
    platform_buffer = Activity(description="Platform Planning Buffer", duration=30, new_tag=3, parent=platform_eco)
    ...
    platform_eco.add_children([website,marketing_platf,bounties, platform_buffer])
    ...
    marketing_company = Activity(description="Marketing", duration=30, new_tag=2, parent=company)
    company_buffer = Activity(description="Organisation Planning Buffer ", duration=20, new_tag=3, parent=company)
    
    # Grandchildren
    auditing = Activity(description="Auditing", duration=10, new_tag=0, parent=organisation)
    hiring = Activity(description="Hiring", duration=20, new_tag=1, parent=organisation)
    administration = Activity(description="Administration", duration=10, new_tag=2, parent=organisation)
    legal = Activity(description="Legal", duration=20, new_tag=3, parent=organisation)
    financial = Activity(description="Financial", duration=20, new_tag=4, parent=organisation)
    organisation.add_children([auditing, hiring, administration, legal, financial])
    
    
    company.add_children([partners,organisation, marketing_company, company_buffer])
