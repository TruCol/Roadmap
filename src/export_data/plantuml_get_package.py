...
def got_java_file(relative_filepath):
    ...
    if not check_if_java_file_exists(relative_filepath):
        ...
        request_file(
            ...
            + "plantuml/files/plantuml.jar/download",
            relative_filepath,
        )
    # Check if the jar file exists after curling it. Raise Exception if it is
    # not found after curling.
    if not check_if_java_file_exists(relative_filepath):
        raise Exception(f"File:{relative_filepath} is not accessible")
    print("Got the PlangUML Java file.")
    return True
...
def request_file(url, output_filepath):
    ...
    :param relative_filepath: The path as seen from the root of this directory,
    in which files are outputted.
...
def run_bash_command(bashCommand):
    ...
    :param bashCommand: A string containing a bash command that
    can be executed.
    """
    # Verbose call.
    # subprocess.Popen(bashCommand, shell=True)
    # Silent call.
    # subprocess.Popen(bashCommand, shell=True, stderr=subprocess.DEVNULL,
