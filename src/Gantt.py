...
class Gantt:
        ...
        for i in range(0, len(self.parents)):
            if i > 0:
                if not self.parents[i].starts_at_child_nr_start is None:
                    ...
                    print(
                        f"parent_descr={self.parents[i].description}, and starts at {self.parents[i].starts_at_child_nr_start} and  tag={self.parents[i].get_tag()}, writing end"
                    )
                    lines.append(
    ...
    def print_costs(self, total_costs, activity, lines):
        lines.append(
            f"'[{activity.description}]  takes: {activity.duration}[days] equating to:{activity.duration*activity.hours_per_day}[hours] and costs: {activity.hourly_wage} per hour, yielding activity costs: {activity.duration*activity.hours_per_day*activity.hourly_wage} Euros."
        )
        total_costs = (
            total_costs
